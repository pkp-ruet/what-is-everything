...
async function readTextFiles(folderPath) {
  try {
    ...
    for (const file of textFiles) {
      ...
      console.log(`✓ Read file: ${file} (${content.length} characters)`);
    }
...
async function saveBlogsToMongoDB(blogs, client) {
  try {
    ...
    console.log(
      `\n✅ Successfully saved ${result.insertedCount} blog(s) to MongoDB`
    );
...
async function main() {
  ...
  try {
    ...
    console.log(`📁 Reading from folder: ${TEXT_FILES_FOLDER}`);
    ...
    console.log(`\n📚 Found ${blogs.length} text file(s) to process`);
    ...
    if (result.insertedCount > 0) {
      ...
      console.log("\n✨ Import completed (no new blogs added)");
    }
  } catch (error) {
